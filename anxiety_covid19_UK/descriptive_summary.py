...
thresholds = [(0, 4), (5, 9), (10, 14), (15, 21)]
...
data_w1['GAD_Threshold'] = data_w1['GAD_Total'].apply(get_gad_threshold)
data_w6['GAD_Threshold'] = data_w6['GAD_Total'].apply(get_gad_threshold)
...
for wave_name, data in [('Wave1', data_w1), ('Wave6', data_w6)]:
    ...
    gad_stats = pd.DataFrame({
        'Variable': ['GAD_Total'],
        'Mean': [data['GAD_Total'].mean().round(2)],
        'SD': [data['GAD_Total'].std().round(2)],
        'Min': [data['GAD_Total'].min()],
        'Max': [data['GAD_Total'].max()],
        'Wave': wave_name,
        'Statistic': 'descriptive_stats'
    })
    
    # 4. Value counts for GAD thresholds
    threshold_counts = data['GAD_Threshold'].value_counts().reset_index()
    threshold_counts.columns = ['GAD_Threshold', 'Count']
...
final_df = final_df[['Wave', 'Statistic', 'Income', 'Variable', 'GAD_Threshold', 
...
print("\n" + "="*50)
print("\nPreview of GAD threshold distribution:")
print("\nWave 1:")
print(data_w1['GAD_Threshold'].value_counts().sort_index())
print("\nWave 6:")
print(data_w6['GAD_Threshold'].value_counts().sort_index())