...
for wa in range(len(waves)):
    ...
    kappa_gad = az.extract(idata.posterior.kappa2, num_samples=1000)["kappa2"].values #(9, 3, 1000)
    ...
    def age_on_gad(gender_idx, question_idx, income_idx, beta_var, var_values):
        ...
        kappa_q = kappa_gad[question_idx, :, :]  # shape: (3, 1000)
    ...
    for q in tqdm(range(Q)):
        for i in range(I):
            for g in range(G):
                aop_ames[g,q,i,:,:] = age_on_gad(g, q, i, bA_y, age_z) / age.std() #recover age scale
    ...
    # Panel 2: Age effects on GAD (aop_ames) - shape: (2, 9, 5, 4, 1000)
