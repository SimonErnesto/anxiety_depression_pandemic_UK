...
for wa in range(len(waves)):
    ...
    kappa_k = az.extract(idata.posterior.kappa_k, num_samples=1000)["kappa_k"].values #(7, 3, 1000)
    alpha_2 = az.extract(idata.posterior.alpha_2, num_samples=1000)["alpha_2"].values #(2, 9, 1000)
    c = az.extract(idata.posterior.c, num_samples=1000)["c"].values # c (2, 7, 1000)
    b = az.extract(idata.posterior.b, num_samples=1000)["b"].values # b (2, 7, 1000)
    ...
    for q in tqdm(range(Q)):
        for i in range(I):
            for g in range(G):
                aop_ames[g,q,i,:,:] = age_on_gad(g, q, i, c, age_z) / age.std() #recover age scale
    ...
    for k_idx, k in enumerate(tqdm(range(1, 5))):  # k_idx: 0,1,2,3 for Q2,Q3,Q4,Q5
        for q in range(Q):
            for g in range(G):
                ...
                ade_result = income_on_gad(g, q, 0, k, age_z)  # shape: (4, 1000)
    ...
    # Panel 2: Age effects on GAD (aop_ames) - shape: (2, 7, 5, 4, 1000)
    ...
    # Panel 3: Income effects on GAD - difference between lowest and highest income
    # iop_ades shape: (2, 7, 4, 1000) - gender, question, score, samples
    ax3 = axes[1, 0]
    ...
    for gender_idx in range(2):
        ...
        for score_idx in range(4):  # For each GAD-7 score
    ...
    # iop_ades2 shape: (2, 7, 4, 1000) - gender, question, income_diff, samples
    ax4 = axes[1, 1]
