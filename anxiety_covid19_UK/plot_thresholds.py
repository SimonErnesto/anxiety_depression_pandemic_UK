...
data_w1['GAD_Total'] = data_w1[gad_columns].sum(axis=1)
data_w6['GAD_Total'] = data_w6[gad_columns].sum(axis=1)

# Load posterior predictive for wave 1
idata_w1 = az.from_netcdf("idata_wave1_anxiety_ordered.nc")
...
idata_w6 = az.from_netcdf("idata_wave6_anxiety_ordered.nc")
...
def calculate_posterior_proportions(pred_scores, data_long):
    ...
    gad7_questions = [f'GAD_{i}' for i in range(1, 8)]
    is_gad = obs_metadata['question'].isin(gad7_questions).values
    ...
    gad_scores = pred_scores[:, is_gad]
    gad_subject_idx = subject_idx[is_gad]
    
    gad7_totals = np.zeros((n_samples, n_subjects))
    for s in range(n_samples):
        gad7_totals[s] = np.bincount(gad_subject_idx, weights=gad_scores[s], minlength=n_subjects)
    ...
    return gad7_totals, subject_incomes, subject_genders

# Calculate posterior for both waves
gad7_w1, incomes_w1, genders_w1 = calculate_posterior_proportions(pred_scores_w1, data_long_w1)
gad7_w6, incomes_w6, genders_w6 = calculate_posterior_proportions(pred_scores_w6, data_long_w6)
...
def calculate_observed_proportions(data):
    ...
    for gender in ['Female', 'Male']:
        ...
        for income in sorted(gender_data['Income'].unique()):
            ...
            props = []
            for low, high in [(0, 4), (5, 9), (10, 14), (15, 21)]:
                in_category = (income_data['GAD_Total'] >= low) & (income_data['GAD_Total'] <= high)
...
def get_group_props(gad7_totals, incomes, genders, gender, income_levels):
...
w1_female_props, w1_female_lower, w1_female_upper = get_group_props(gad7_w1, incomes_w1, genders_w1, 'Female', income_levels)
