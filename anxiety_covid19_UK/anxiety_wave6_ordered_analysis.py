...
with pm.Model(coords=coords) as mod:
    ...
    al_w_z = pm.Normal("al_w_z", 0, 1, dims=("gender")) #intercept mediator
    al_w_l = pm.Normal("al_w_l", 0, 1)
    al_w_s = pm.HalfNormal("al_w_s", 1)
    alpha_w = pm.Deterministic("alpha_w", al_w_l + al_w_s*al_w_z)
    
    al_y_z = pm.Normal("al_y_z", 0, 1, dims=("gender","question")) #intercept total
    al_y_l = pm.Normal("al_y_l", 0, 1)
    al_y_s = pm.HalfNormal("al_y_s", 1)
    alpha_y = pm.Deterministic("alpha_y", al_y_l + al_y_s*al_y_z)

    bA_w_z = pm.Normal("bA_w_z", 0, 1, dims=("gender")) #age slope mediator
    bA_w_l = pm.Normal("bA_w_l", 0, 1)
    bA_w_s = pm.HalfNormal("bA_w_s", 1)
    bA_w = pm.Deterministic("bA_w", bA_w_l + bA_w_s*bA_w_z)
    
    bA_y_z = pm.Normal("bA_y_z", 0, 1, dims=("gender","question")) #age slope
    bA_y_l = pm.Normal("bA_y_l", 0, 1)
    bA_y_s = pm.HalfNormal("bA_y_s", 1)
    bA_y = pm.Deterministic("bA_y", bA_y_l + bA_y_s*bA_y_z)
    
    bI_z = pm.Normal("bI_z", 0, 1, dims=("gender","question")) #income slope
    bI_l = pm.Normal("bI_l", 0, 1)
    bI_s = pm.HalfNormal("bI_s", 1)
    bI = pm.Deterministic("bI", bI_l + bI_s*bI_z)
...
plt.suptitle("Wave-6 Prior Predictive Checks Anxiety", size=18)
plt.tight_layout()
...
az.to_netcdf(idata, "idata_wave6_anxiety_ordered.nc")
...
plt.suptitle("Wave-6 Posterior Predictive Checks Anxiety", size=18)
plt.tight_layout()
