...
def age_on_gad(gender_idx, income_idx, beta_var, var_values):
    """
...
for i in tqdm(range(I)):
    for g in range(G):
        aop_ames_wave1[g,i,:,:] = age_on_gad(g, i, c, age_z) / age.std() #recover age scale



data = pd.read_csv("./data/anxiety_covid19_UK_wave6_data.csv")
idata = az.from_netcdf("./idata_wave6_anxiety_ordered.nc")
...
for i in tqdm(range(I)):
    for g in range(G):
        aop_ames_wave6[g,i,:,:] = age_on_gad(g, i, c, age_z) / age.std() #recover age scale
...
for w in range(2):
    wave = waves[w]
    data = pd.read_csv("./data/anxiety_covid19_UK_"+wave+"_data.csv")
    
    idata = az.from_netcdf("./idata_"+wave+"_anxiety_ordered.nc")
    ...
    def income_on_gad(gender_idx, income_j, income_k, age_values):
        """
    ...
    for k_idx, k in enumerate(tqdm(range(1, 5))):  # k_idx: 0,1,2,3 for Q2,Q3,Q4,Q5
        for g in range(G):
            ...
            ade_result = income_on_gad(g, 0, k, age_values)  # shape: (4, 1000)
...
plt.tight_layout()
plt.savefig("anxiety_ades_continous_score.png", dpi=300)
plt.show()
plt.close()
...
plt.tight_layout()
plt.savefig("anxiety_ades_continous_income.png", dpi=300)
plt.show()
plt.close()