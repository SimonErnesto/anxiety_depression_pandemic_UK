...
# Calculate observed PHQ-7 totals
dep_columns = [f'Dep_{i}' for i in range(1, 10)]
data_w1['Dep_Total'] = data_w1[dep_columns].sum(axis=1)
data_w6['Dep_Total'] = data_w6[dep_columns].sum(axis=1)

# Load posterior predictive for wave 1
...
# Load posterior predictive for wave 6
...
def calculate_posterior_proportions(pred_scores, data_long):
    ...
    dep7_questions = [f'Dep_{i}' for i in range(1, 10)]
    is_dep = obs_metadata['question'].isin(dep7_questions).values
    ...
    dep7_totals = np.zeros((n_samples, n_subjects))
    for s in range(n_samples):
        dep7_totals[s] = np.bincount(dep_subject_idx, weights=dep_scores[s], minlength=n_subjects)
    ...
    return dep7_totals, subject_incomes, subject_genders

# Calculate posterior for both waves
dep7_w1, incomes_w1, genders_w1 = calculate_posterior_proportions(pred_scores_w1, data_long_w1)
dep7_w6, incomes_w6, genders_w6 = calculate_posterior_proportions(pred_scores_w6, data_long_w6)
...
def calculate_observed_proportions(data):
    ...
    for gender in ['Female', 'Male']:
        ...
        for income in sorted(gender_data['Income'].unique()):
            ...
            props = []
            observed_max = data_w1['Dep_Total'].max()  # 24
            for low, high in [(0, 4), (5, 9), (10, 14), (15, 19), (20, observed_max)]:
...
colors = ['#2ecc71', '#f39c12', '#e67e22',   '#e74c3c', '#e0200c']
income_levels = sorted(data_w1['Income'].unique())

# Function to get posterior proportions by group (using 90% HDI)
def get_group_props(dep7_totals, incomes, genders, gender, income_levels):
    ...
    for inc_idx, income in enumerate(income_levels):
        ...
        if len(subject_indices) > 0:
            observed_max = data_w1['Dep_Total'].max()  # 24
            for thresh_idx, (low, high) in enumerate([(0, 4), (5, 9), (10, 14), (15, 19), (20, observed_max)]):
                in_category = (dep7_totals[:, subject_indices] >= low) & (dep7_totals[:, subject_indices] <= high)
...
w1_female_props, w1_female_lower, w1_female_upper = get_group_props(dep7_w1, incomes_w1, genders_w1, 'Female', income_levels)
w1_male_props, w1_male_lower, w1_male_upper = get_group_props(dep7_w1, incomes_w1, genders_w1, 'Male', income_levels)
w6_female_props, w6_female_lower, w6_female_upper = get_group_props(dep7_w6, incomes_w6, genders_w6, 'Female', income_levels)
w6_male_props, w6_male_lower, w6_male_upper = get_group_props(dep7_w6, incomes_w6, genders_w6, 'Male', income_levels)
